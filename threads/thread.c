...
#include "intrinsic.h"
#include "threads/fixed_point.h"			//mlfqs 부동소수점 header
#ifdef USERPROG
...
//mlfqs에서 사용할 변수
//nice, recent_cpu, load_avg default 정의
...
thread_awake(int64_t ticks)
{
  ...
  while (e != list_end (&sleep_list)){
	...
	//thread wake up time 초기화 해주는 거도 있으면 좋음!!!!

      e = list_remove (&t->elem);	// sleep list 에서 제거
...
thread_start (void) {
	...
	load_avg = LOAD_AVG_DEFAULT;			//mlfqs load_avg 초기화
...
thread_create (const char *name, int priority,
	...
	/*compare priority*/
	//preemption 방식도 좋으나 그냥 yield하는 것도 같은 방식이다.
	thread_preemption();
...
donate_priority(){
	...
	for(depth = 0; depth < 8; depth++){
		...
		struct thread *chg = cur->wait_on_lock->holder;
		chg->priority = cur->priority;
		cur = chg;
	}
}
...
thread_set_priority (int new_priority) {
	...
	struct thread *cur = thread_current();
	// int cur_last_priority = cur->priority;				//이전부분 mlfqs 이후 주석 처리 이유는?
	// cur->priority = new_priority;
	cur->init_priority = new_priority;
	rebuild_priority();

	//if(cur_last_priority != cur->priority)
		donate_priority();
...
thread_get_recent_cpu (void) {
	...
	enum intr_level old_level = intr_disable();
	struct thread *cur = thread_current();
	int recent_cpu_val = fp_to_int_round(mult_mixed(cur->recent_cpu, 100));
	intr_set_level(old_level);
...
init_thread (struct thread *t, const char *name, int priority) {
	...
	//현재 thread에 대한 초기 priority 값 저장
	//wait_on_clock 변수
	//donations list 초기화
...
schedule (void) {
	...
	if (curr != next) {
		...
		if (curr && curr->status == THREAD_DYING && curr != initial_thread) {
			...
			list_remove(&curr->all_elem);//mlfqs관련 변경
